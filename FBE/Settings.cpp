...
CSettings::CSettings()
    : m_need_restart(false), keycodes(0)
{
}
	...
	for (int i = 1; i < 100; ++i)
	{
		...
		if (mapSymbs.Lookup(pattern) != L"")
		{
			...
			if ((val >= 0x0300 && val <= 0x036F) || (val >= 0x1DC0 && val <= 0x1DFF) ||
			    (val >= 0x20D0 && val <= 0x20FF) || (val >= 0xFE20 && val <= 0xFE2F))
				desc += CString(L"( ");
			else
				desc += CString(L"(");

			if (val == 160)
				desc += GetNBSPChar() + CString(L")");
			else
				desc += (wchar_t)val + CString(L")");
...
CHotkey::CHotkey(CString reg_name, int IDS_CMD_NAME, BYTE fVirt, WORD cmd, WORD key, CString descr)
{
	m_reg_name = reg_name;
...
CHotkey::CHotkey(CString reg_name, int IDS_CMD_NAME, CString uchar, BYTE fVirt, WORD cmd, WORD key, CString descr)
{
	m_reg_name = reg_name;
	m_name.LoadString(IDS_CMD_NAME);
	m_name += uchar;
...
CHotkey::CHotkey(CString reg_name, CString name, wchar_t symbol, BYTE fVirt, WORD cmd, WORD key, CString descr)
{
	m_reg_name = reg_name;
	m_name = name;
...
CHotkey::CHotkey(CString reg_name, CString cmd_name, BYTE fVirt, WORD cmd, WORD key, CString descr)
{
	m_reg_name = reg_name;
	m_name = cmd_name;
...
int CHotkey::GetProperties(std::vector<CString> & properties)
{
...
bool CHotkey::GetPropertyValue(const CString & sProperty, CProperty & property)
{
	if (sProperty == L"Name")
	{
		property = m_reg_name;
...
bool CHotkey::SetPropertyValue(const CString & sProperty, CProperty & sValue)
{
	if (sProperty == L"Name")
	{
...
bool CHotkey::HasMultipleInstances()
{
	return false;
}

CString CHotkey::GetClassName()
{
	return L"Hotkey";
}

CString CHotkey::GetID()
{
	return L"";
}

ISerializable * CHotkey::Create()
{
	return new CHotkey;
}

void CHotkey::Destroy(ISerializable * obj)
{
	delete obj;
}

CHotkeysGroup::CHotkeysGroup()
{
}

CHotkeysGroup::CHotkeysGroup(CString reg_name, int IDS_GROUP_NAME)
{
	m_reg_name = reg_name;
...
int CHotkeysGroup::GetProperties(std::vector<CString> & properties)
{
...
bool CHotkeysGroup::GetPropertyValue(const CString & sProperty, CProperty & property)
{
...
bool CHotkeysGroup::SetPropertyValue(const CString & sProperty, CProperty & sValue)
{
	if (sProperty == L"GroupName")
	{
		m_reg_name = sValue.GetStringValue();
...
bool CHotkeysGroup::HasMultipleInstances()
{
	return true;
}

CString CHotkeysGroup::GetClassName()
{
	return L"HkGroup";
}

CString CHotkeysGroup::GetID()
{
	return L"";
}

ISerializable * CHotkeysGroup::Create()
{
	return new CHotkeysGroup;
}

void CHotkeysGroup::Destroy(ISerializable * obj)
{
	delete obj;
}
